...
int main() {
  ...
  const char *kernelSource = readKernelSourceFromFile(
      "7031ff188e305ce0cb4dcbe8238c21cf68a9f1f69573238c3ca49bb7d0d2372b.cl");
  ...
  cl_mem f_buffer = NULL, b_buffer = NULL;
  size_t gsize = 32, lsize = 16;
  int f_bound = (int)ceil(1.0000000000000002 * gsize -
                          5.398695250599143e-16 * lsize - 1.0000000000004547);
  int b_bound = (int)ceil(1.39 * gsize - 5.3986952505 * lsize + 123.2);
  ...
  clEnqueueReadBuffer(queue, b_buffer, CL_TRUE, 0, sizeof(float) * b_bound, b,
                      0, NULL, NULL);

  printf("b after: \n");
  for (int i = 0; i < b_bound + 20; i++) {
    printf("b[%d]: %lf\n", i, b[i]);
  }
