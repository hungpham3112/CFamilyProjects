...
__global__ void computeGPU(DATATYPECADNAGPUF *a, DATATYPECADNAGPUF *b){
    ...
    if(i<N){
        ...
        for(int j=0; j<NLOOP; j++){
            f = __float_gpu_st2half_gpu_st(b[i])* f;
            //f = __float_gpu_st2half_gpu_st(b[i]) * f;
	    printf("a = %f, b = %f f = %f \n",(float)a[i], (float)b[i],  __half_gpu_st2float(f));
        }
	 ...
	 if(i<N){
        ...
        for(int j=0; j<NLOOP; j++){
            f = __float2half(b[i]) *  f;
            //f = __float_gpu_st2half_gpu_st(b[i]) * f;
	    printf("a = %f, b = %f f = %f \n",(float)a[i], (float)b[i],  __half2float(f));
        }
        a[i] = __half2float(f);
    }
...
int main(int argc, char **argv){
    ...
    FILE* fic=fopen("addCombBX.res", "a");
