  ...
  printf("- Initialization\n");
    ...
    for (int i = 0; i < size; i++) {
      // Keep the numbers small so we don't get truncation error in the sum
        h_idata[i] = (float_st)rand() / RAND_MAX;
    }
    ...
    float_st *d_idata = NULL;
    float_st *d_odata = NULL;

    cudaMalloc((void **)&d_idata, bytes);
    cudaMalloc((void **)&d_odata, numBlocks * sizeof(float_st));

  cudaMemcpyFromSymbol(d_idata,h_idata,sizeof(float_st) * size,0,cudaMemcpyHostToDevice);
  // cudaMemcpy(d_odata, h_idata,sizeof(float) * numBlocks,cudaMemcpyHostToDevice);

  reduce(size, numThreads, numBlocks, d_idata, d_odata);

  cudaMemcpyFromSymbol(h_odata, d_odata, sizeof(float_st)* numBlocks, 0, cudaMemcpyDeviceToHost);
    float gpu_result =
    ...
    int precision = 8;
      printf("\nGPU result = %.*f\n", precision, (double_st)gpu_result);
      printf("CPU result = %.*f\n\n", precision, (double_st)cpu_result);
