...
__device__ double_gpu_st X_GPU[VECTOR_SIZE];
__device__ double_gpu_st Y_GPU[VECTOR_SIZE];
__device__ double_gpu_st S_GPU[VECTOR_SIZE];



double_st X_CPU[VECTOR_SIZE];
double_st Y_CPU[VECTOR_SIZE];
double_st S_CPU[VECTOR_SIZE];
double_st S_CPU_fromGPU[VECTOR_SIZE];
...
void InitCPU(void)
{
 // Init vectors in order to have: S=2.X+Y == VECTOR_SIZE
  ...
  X_CPU[0] = (double)5.5;
  Y_CPU[0] = (double)61./11;
  S_CPU[0] = (double)1051.0;


  for (int i = 1; i < VECTOR_SIZE; i++){
    X_CPU[i] = (double)i;
    Y_CPU[i] = (double)i+1;
    S_CPU[i] = (double)1051.0;
  }

  // Set the GPU table of results to -1.0 (just to improve debugging)
  for (int i = 0; i < VECTOR_SIZE; i++) {
    S_CPU_fromGPU[i] = (double)-1.0;
  }
  cudaMemcpyToSymbol(S_GPU,&S_CPU_fromGPU[0],sizeof(double_st)*VECTOR_SIZE,0,cudaMemcpyHostToDevice);

  // Set the CPU buffer of GPU results to -2.0 (just to improve debugging)
  for (int i = 0; i < VECTOR_SIZE; i++){
    S_CPU_fromGPU[i] = (double)-2.0;
  }
}
...
{
  cudaMemcpyToSymbol(X_GPU,&X_CPU[0],sizeof(double_st)*VECTOR_SIZE,0,
		     cudaMemcpyHostToDevice);
  cudaMemcpyToSymbol(Y_GPU,&Y_CPU[0],sizeof(double_st)*VECTOR_SIZE,0,
		     cudaMemcpyHostToDevice);
}
...
{
  cudaMemcpyFromSymbol(&S_CPU_fromGPU[0],S_GPU,sizeof(double_st)*VECTOR_SIZE,0,
