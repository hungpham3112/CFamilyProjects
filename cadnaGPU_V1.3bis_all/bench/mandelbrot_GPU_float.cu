...
__host__ void mandelbrotCPU(int *buf, int width, int height, float x0, float y0, float dx, float dy, int maxIterations){
  for (int j = 0; j < height; j++) {
	...
	DATATYPECADNA new_im = DATATYPECADNA(2.f * z_re * z_im);
	z_re = x + new_re;
	z_im = y + new_im;
      }
