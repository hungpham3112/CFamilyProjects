...
__global__ void mandelbrotKernel(int *d_buf, int width, int height, float x0, float y0, float dx, float dy, int maxIterations){
  ...
  cadna_init_gpu();
  if (i < width && j < height){
    DATATYPECADNAGPU x = __float2half2_gpu_st(x0 + i * dx);
    DATATYPECADNAGPU y = __float2half2_gpu_st(y0 + j * dy);
    int k;
...
//	printf("x = %f y = %f z_re = %f z_im = %f\n", __half_gpu_st2float(x), __half_gpu_st2float(y), __half_gpu_st2float(z_re), __half_gpu_st2float(z_im));
    for (k = 0; k < maxIterations; ++k) {
      if (z_re * z_re + z_im * z_im > (__float2half2_gpu_st((DATATYPE)4.f)))
      {
	      break;
      }
      DATATYPECADNAGPU new_re = z_re*z_re - z_im*z_im;
      DATATYPECADNAGPU new_im = (__float2half2_gpu_st((DATATYPE)2.f)) * z_re * z_im;
      z_re = x + new_re;
      z_im = y + new_im;
...
#else
  if (i < width && j < height){
    DATATYPECADNAGPU x = __float2half2_rn(x0 + i * dx);
    DATATYPECADNAGPU y = __float2half2_rn(y0 + j * dy);
    int k;
    DATATYPECADNAGPU z_re = x;
    DATATYPECADNAGPU z_im = y;

    for (k = 0; k < maxIterations; ++k) {
      if ( __hbgt2(__hadd2(__hmul2(z_re , z_re),  __hmul2(z_im , z_im)) , __float2half2_rn((DATATYPE)4.f)))
	break;
     // DATATYPECADNAGPU new_re = z_re*z_re - z_im*z_im;
      DATATYPECADNAGPU new_re = __hsub2(__hmul2(z_re , z_re),  __hmul2(z_im , z_im));
      //DATATYPECADNAGPU new_im = __float2half2_rn((DATATYPE)2.f) * z_re * z_im;
      DATATYPECADNAGPU new_im = __hmul2((__hmul2(__float2half2_rn((DATATYPE)2.f) , z_re)) , z_im);
      z_re = __hadd2(x , new_re);
      z_im = __hadd2(y , new_im);
    }
...
#ifdef CADNA
  cadna_init_gpu();
  if (i < width && (i%2 == 0 || j %2 == 0) && j < height){
    DATATYPECADNAGPU x = __floats2half2_gpu_st(x0 + i * dx, x0 + (i+1) * dx);
    DATATYPECADNAGPU y = __floats2half2_gpu_st(y0 + j * dy, y0 + (j+1) * dy);
    int k;
    DATATYPECADNAGPU z_re = x;
    DATATYPECADNAGPU z_im = y;
//	printf("x = %f y = %f z_re = %f z_im = %f\n", __half_gpu_st2float(x), __half_gpu_st2float(y), __half_gpu_st2float(z_re), __half_gpu_st2float(z_im));
...
#else
  if (i < width && (i%2 == 0 || j %2 == 0)&& j < height){
    DATATYPECADNAGPU x = __floats2half2_rn(x0 + i * dx, x0 + (i+1) * dx);
    DATATYPECADNAGPU y = __floats2half2_rn(y0 + j * dy, y0 + (j+1) * dy);
    int k;
    DATATYPECADNAGPU z_re = x;
    DATATYPECADNAGPU z_im = y;
