...
BOOTDEV_SRC="${BOOTDEV_SRC:-$ROOT/tools/bootdev/bootdev.c}"

if [ ! -f "$BOOTDEV_SRC" ] && [ -f "$ROOT/scripts/bootdev.c" ]; then
  BOOTDEV_SRC="$ROOT/scripts/bootdev.c"
fi

if [ ! -f "$BOOTDEV_SRC" ]; then
  echo "error: missing bootdev source" >&2
  echo "expected one of:" >&2
  echo "  $ROOT/tools/bootdev/bootdev.c" >&2
  echo "  $ROOT/scripts/bootdev.c" >&2
  exit 2
fi

mkdir -p "$ROOT/build"
mkdir -p "$ROOT/include"
mkdir -p "$ROOT/third_party/munit"
...
cc -std=c17 -Wall -Wextra -O2 "$BOOTDEV_SRC" -o "$ROOT/build/bootdev"
sudo install -m 0755 "$ROOT/build/bootdev" /usr/local/bin/bootdev

echo "installed: /usr/local/bin/bootdev"
echo "root:      $ROOT"
echo "source:    $BOOTDEV_SRC"
