...
int main() {
  ...
  const char *kernelSource = readKernelSourceFromFile(
      "9772f6ac7619740c33d16a1d6b6e3b746bde383837dbd42e13da63b1ff1b75e7.cl");
  ...
  int s_bound = (int)ceil(-2.3216713841507802e-05 * gsize +
                          0.0003298893497292032 * lsize + 1.0144897959183674);
  int *s = (int *)malloc(sizeof(int) * s_bound);
  printf("s_bound is: %d \n", s_bound);
  generateRandomIntArray(s, s_bound, -100, 100);
  printf("s before: \n");
  ...
  s_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                            sizeof(int) * s_bound, s, &err);
  if (err != CL_SUCCESS) {
    perror("Couldn't create buffer");
    exit(1);
  }
  clEnqueueWriteBuffer(queue, s_buffer, CL_TRUE, 0, sizeof(int) * s_bound, s, 0,
                       NULL, NULL);
  ...
  clEnqueueReadBuffer(queue, s_buffer, CL_TRUE, 0, sizeof(int) * s_bound, s, 0,
                      NULL, NULL);
